...
start_backend() {
    ...
    python ${REPO_DIR}/scripts/counter.py override 2>&1 > ${output}/counter.log &
}

# Function to start the backend application
continue_backend() {
    echo "Continue the backend counter..."
    pkill -f ${REPO_DIR}/scripts/counter.py
    python ${REPO_DIR}/scripts/counter.py 2>&1 >> ${output}/counter.log &
}
...
stop_backend() {
    ...
    pkill -f ${REPO_DIR}/scripts/counter.py
    python ${REPO_DIR}/scripts/counter.py draw
}
