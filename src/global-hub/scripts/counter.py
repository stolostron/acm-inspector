...
def get_conn():
    ...
    secret = v1.read_namespaced_secret(
        name="multicluster-global-hub-postgres", namespace=global_hub_namespace
    )
    postgres_dict = {
        key: base64.b64decode(val).decode("utf-8") for key, val in secret.data.items()
    }
    print(postgres_dict)
    ...
    service = v1.read_namespaced_service(
        name="multicluster-global-hub-postgres-lb", namespace=global_hub_namespace
    )
...
def record_initial(override):
    ...
    while True:
        ...
        if withHeader and override:
            df.to_csv(
                output_path + "/count-initialization.csv", header=True, index=False
            )
            withHeader = False
        else:
            df.to_csv(
                output_path + "/count-initialization.csv",
                mode="a",
                header=False,
                index=False,
            )
...
def record_compliance(override):
    ...
    while True:
        ...
        if withHeader and override:
            ...
            df.to_csv(
                output_path + "/count-compliance.csv",
                mode="a",
                header=False,
                index=False,
            )
...
def draw():
    ...
    dates = [datetime.strptime(d, "%Y-%m-%d %H:%M:%S") for d in df["time"]]
    ax.plot(
        dates, df["compliant"], "-", color="green", label="Compliant", linewidth=2.0
    )
    ax.plot(
        dates,
        df["non_compliant"],
