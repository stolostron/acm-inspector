...
def checkGlobalHubMemory(start_time, end_time, step):
    print(Back.GREEN + "")
    print(
        "============================================================================================="
    )
...
def global_hub_total_memory_usage(pc, start_time, end_time, step):
    ...
    try:
        ...
        global_hub_trend_df.index = pandas.to_datetime(
            global_hub_trend_df.index, unit="s"
        )
        global_hub_trend_df = global_hub_trend_df.sort_index(ascending=True)
        global_hub_trend_df.rename(
            columns={"value": "GlobalHubMemUsageRSSGB"}, inplace=True
        )
        ...
        global_hub_trend_df.to_csv(
            output_path + "/" + file + ".csv", index=True, header=True
        )
        plt.close("all")
